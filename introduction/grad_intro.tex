...
\eeq 
To leading order, and with the choice of $\lambda=1$, it was shown in \cite{luscher_properties_2010} that \cref{flow_mod} reduces to:
\beq
...
\fig[0.15]{introduction/topcflow.png}{Topological charge computed ad one euclidean time of a lattice of size $32^3\times64$ with lattice spacing $0.06793$ fm. The flow different plots are for flow-times $\sqrt{8t_f} = {0, 0.14, 0.30}$ fm on the first row and $\sqrt{8t_f} = {0.43 , 0.52, 0.60}$ fm on the second row.}{visit:topcflow}
...
\end{align}
with the aid of these coefficients the only unknown variable in \cref{alpha:M} is the momentum scale, so by fitting this function to the lattice over a range of energies where some degree of overlap is found leads to an estimation of $\Lambda$. 