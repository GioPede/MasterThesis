...
\beq
    S_E[x(t)] = \int_{t_i}^{t_f} dt \Lagr [x(t)] = \int_{t_i}^{t_f} dt \frac{m}{2}\dot x(t)^2 +  V[x(t)].
\eeq
...
\subsection{The Wilson Gluonic Action}
...
To express expectation values and correlators on the lattice we employ the path integral formalism. The partition function, as we have seen earlier, is the path integral of the fields over the whole space of the action. For the case of QCD the fields are $U$, $\psi$ and $\bpsi$:
\beq
...
\begin{figure}[!htb]
    ...
    \end{tikzpicture}
    \capt{Schematic representation of the symmetric definition of the clover term $G_{\mu\nu}^{(clover)}$ on the lattice. The field strength tensor for a plane $\mu\nu$ at a lattice site is given by the sum of all the for plaquettes that start from that site, all taken with the same orientation.}
    \label{fig:clover}
\end{figure}
